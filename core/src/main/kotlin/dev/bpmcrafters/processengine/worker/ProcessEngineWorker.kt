 ...
 *
 * The attributes `topic` and `value` are aliases of each other: specify either of them, but not both with different values.
 * @since 0.0.1
 */
@MustBeDocumented
...
annotation class ProcessEngineWorker(
  /**
   * Topic name to subscribe this worker for. Alias for `value`.
   */
  val topic: String = DEFAULT_UNSET_TOPIC,
  /**
   * Topic name to subscribe this worker for. Alias for `topic`.
   */
